    ...
    def data(self, index, role=Qt.ItemDataRole.DisplayRole):
        ...
        if role == Qt.ItemDataRole.DisplayRole:
            if col == 1:
                ...
                if dl.total_bytes > 0:
                    mb_downloaded = dl.downloaded_bytes / (1024 * 1024)
                    mb_total = dl.total_bytes / (1024 * 1024)
                    if dl.status == DownloadItem.STATUS_COMPLETED:
                        return f"{mb_total:.1f} MB"
                    return f"{mb_downloaded:.1f} / {mb_total:.1f} MB"
                else:
                    mb_downloaded = dl.downloaded_bytes / (1024 * 1024)
                    return f"{mb_downloaded:.1f} MB"
            elif col == 4:
                ...
                if dl.status == DownloadItem.STATUS_DOWNLOADING:
                    ...
                    if eta >= 0:
                        ...
                        return "Calculating..."
                elif dl.status == DownloadItem.STATUS_COMPLETED:
                    return "Done"
                return ""
            elif col == 6:
                return dl.status
